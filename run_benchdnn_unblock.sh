...
./benchdnn --conv --engine=$engine:$device --fast-ref=true --mem-check=false --mode=f \
           --mode-modifier=p --repeats-per-prb=$reps --fix-times-per-prb=$iter \
           --perf-template=%driver%,%engine%,%-time%,%0time%,%+time%,%0ctime% --stag=abc \
           --wtag=abc --dtag=abc --attr-scratchpad=user --attr-post-ops=eltwise_relu \
           --dir=FWD_I \
...
./benchdnn --ip --engine=$engine:$device --fast-ref=true --mem-check=false --mode=f \
           ...
           --wtag=ab --dtag=ab --attr-scratchpad=user --attr-post-ops=eltwise_relu --dir=FWD_I \
